...
def image_links_to_som_from_csv(csv_input_file, som):
    ...
    with open(csv_input_file, 'rb') as f:
        ...
        # cover the case where the file contains som coordinates and a column with image filename
        for row in csv_content:
            ...
            # cover the case where the file contains som coordinates and plate, mjd, fiberid columns
            # uses BOSS filename scheme only
            if ('som_x' in row and 'som_y' in row and 'mjd' in row and 'plate' in row and 'fiberid' in row):
                plate = ''.join(('0000', row['plate']))
                plate = plate[-4:]
                fiberid = ''.join(('0000', row['fiberid']))
                fiberid = fiberid[-4:]
                mjd = row['mjd']
                image_file_path = str(plate) + '/' + '-'.join( ('spec', plate, mjd, fiberid) ) + '.fits.png'
                som.set_som_element(int(row['som_x']),int(row['som_y']),image_file_path)
...
if __name__ == '__main__':
    ...
    exclusive_input_group.add_argument("-C", "--csvfile", type=str, help="A csv file containing columns with header x,y,data for loading a som. Expects space as delimiter.")
    ...
    if (args.csvfile != None):
        image_links_to_som_from_csv(args.csvfile, som_icons)
